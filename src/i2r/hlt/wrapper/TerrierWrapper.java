...
import at.ac.fhkufstein.QueryResult;
...
/**
 * This class is the wrapper on top of terrier API. Use it for the most of IR
 * related needs.
 *
...
public class TerrierWrapper {
    ...
    /**
     * It is used across termpipeline related functions to load proper stemmer,
     * stopword file etc..
     */
    ...
    public ArrayList<QueryResult> query(String term) throws IOException {
        Manager queryingManager = new Manager(index);

        SearchRequest srq = queryingManager.newSearchRequest("queryID"+(nextQueryId++), term);
        ...
        for (int index = 0; index < rs.getDocids().length; index++) {
            queryResults.add(new QueryResult(rs.getDocids()[index], rs.getScores()[index]));
        }
        return queryResults;
    }

    /**
     * Sets the stopword removal and points to the stopwords file at
     * "$terrier_home/share/" location. Make sure that language files is
     * present.
     *
    ...
    /**
     * Sets the stemmer environment of the Terrier. Makes an object of the
     * TermPipeline and sets it with the stemmer. The Stemmer is Snowball and
     * the possible laguages are English, Danish, Dutch, German, French,
     * Finnish, Italian, Hugarian, Norwegian, Portuguese, Romanian, Russian,
    ...
    /**
     * This class tries to act as a customized user interface to the terrier in
     * order to get the ranked list for given query and already built index with
     * the specified model with/without query expansion.
     *
     * @param query = query in the String format (Make sure the query is
     * tokenized already)
     * @param match_model = Matching model like [TF_IDF, BM25, InL2,
     * LemurTF_IDF, PL2, DFR_BM25]
    ...
    public ResultSet getResultSet(String query,
        ...
        /**
         * if you need top N results then set it to "N". "0" means all the
    ...
    /**
     * Stems using the Snowball Stemmer for aforementioned Language in the
     * setStemmer().
     *
    ...
    /**
     * Converts a file into query. Useful when a complete document is used as a
     * query.
     *
     * @param f Input File
    ...
    /**
     * This method returns a \<Integer, String\> map with Integer corresponding
     * to the filename String. Also creates "$prefix.docid.map" file with this
     * information at the index path.
     *
     * @param path = path to the collection.spec file used for generating the
     * index.
     */
    ...
    /**
     * Returns the normalised TF of the 'term' devided by total tokens in the
     * index
     *
     * @param term
     * @return
    ...
    /**
     * Gives maximun TF within the document. Useful when collecting document
     * level probabilities of the terms.
     *
     * @param docid
     * @return
     * @throws IOException
    ...
    /**
     * Return the 'IDF' of the 'term'. Calculates based on the formula log(N/df)
     * where N = total documents in the collection.
     *
     * @param term
     * @return
     */
    ...
    /**
     * To get the vector of the given document as TF-IDF score or Word Count.
     * Binary Vector can easily be made based on the returned vector.
     *
     * @param f Input File
     * @param wordCount
     * @return
    ...
    /**
     * This method prepares a word count or real valued vector for the input
     * docid. The word count vector is if a term is present with what frequency,
     * while the real vector is the TF-IDF of the terms present in the document.
     *
     * @param docid
     * @param wordCount
     * @return
    ...
    /**
     * Loads the IDF of all the terms in the index and set the maxIDF parameter
     * for future use.
     *
     */
    ...
    /**
     * Calculates the TF-IDF scores of the terms of the specified document and
     * returns top N terms in an array.
     *
     * @param docid
     * @param N
     * @return
    ...
    public String[] topTerms(int docid, int N) throws IOException {
        ...
        IterablePosting postings = this.di.getPostings((BitIndexPointer) this.doi.getDocumentEntry(docid));
        while (postings.next() != IterablePosting.EOL) {
    ...
    /**
     * This method creates the index of type, Anyway it considers the collection
     * to be SimpleFileCollection i.e. plain text. TODO Add support for other
     * collection classes.
     *
     * @param path = Path to create the index
     * @param prefix = prefix to use for index
     * @param collection_type = Type of the collection
     * {@value "SimpleFileCollection"}
     * @throws IOException
     */
    ...
    /**
     * This method prepares the list of files present in the directory to be
     * used for indexing. Basically creates "collection.spec" file used by
     * terrier to index.
     *
    ...
    /**
     * Generates a map of terms in the document and its within document term
     * frequencies.
     *
     * @param docid
     * @return
...
//				weight = this.getTfOPCA(le.getKey()) * this.getIdfOPCA(le.getKey(), 2*n);
    ...
    /**
     * This method returns the expanded query with specified pseudo relevance
     * feedback model and relevance model. For possible qeModel see
...
/**
 * This is to sort the hash maps based on its values (type Double). If you wish
 * to change it for other types like (Integer, Float etc.) then change the
 * casting. This is currently sorts the hashmap in decreasing order like
 * ranklist. If you need to change it to ascending order then reverse the
 * "/<" sign to "/>".
 *
